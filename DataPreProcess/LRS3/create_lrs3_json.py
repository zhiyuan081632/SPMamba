#!/usr/bin/env python
"""
Create JSON manifests for the LRS3-2mix dataset, mirroring the Echo2Mix
manifest format so that Echo2MixDataModule can be reused directly.

Each split directory receives three files:
  mix.json   – [[filepath, num_samples], ...]
  s1.json    – [[filepath, num_samples], ...]
  s2.json    – [[filepath, num_samples], ...]

Usage
-----
    python create_lrs3_json.py \
        --src_root /mnt/e/data/LRS3/LRS3-2mix \
        --out_root /mnt/d/project/prjANS/src/AVSE/SPMamba/DataPreProcess \
...
DEFAULT_SRC_ROOT = Path("/mnt/e/data/LRS3/2mix")
...
def main():
    parser = argparse.ArgumentParser(
        description="Create LRS3-2mix JSON manifests for SPMamba.")
    parser.add_argument("--src_root", type=Path, default=DEFAULT_SRC_ROOT,
                        help="LRS3-2mix wav root containing train/val/test.")
    parser.add_argument("--out_root", type=Path, default=DEFAULT_OUT_ROOT,
                        help="Output root. Defaults to SPMamba/DataPreProcess.")
    ...
    for split in args.splits:
        ...
        if not split_dir.is_dir():
            print(f"[Warning] Missing split directory: {split_dir}, skipping.")
            continue
